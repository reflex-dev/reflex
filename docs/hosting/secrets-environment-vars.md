...
```python eval
rx.image(src="https://web.reflex-assets.dev/other/environment_variables.webp", alt='Environment variables panel in Reflex Cloud UI')
```

Then click on the `Secrets` tab as shown below.

```python eval
rx.image(src="https://web.reflex-assets.dev/other/environment_variables_2.webp", alt='Adding environment variables in Reflex Cloud UI')
```
