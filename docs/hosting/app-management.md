...
rx.image(
    ...
    alt='Stopping an app in Reflex Cloud', padding_bottom="20px"
)
```
...
```python eval
rx.image(src="https://web.reflex-assets.dev/other/environment_variables.webp", alt='App environment variables in Reflex Cloud')
```

Then click on the `Danger` tab as shown below.

```python eval
rx.image(src="https://web.reflex-assets.dev/other/deleting_app.webp", alt='Deleting an app in Reflex Cloud')
```
