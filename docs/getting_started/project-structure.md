...
Reflex copies these files into `.web/public` during compilation. Deleting or renaming an asset removes its previous copy on the next compile, including production exports, while preserving public files generated by plugins. Files already deleted before upgrading to a version with asset tracking require a one-time removal of the stale copies or a clean `.web` directory.
