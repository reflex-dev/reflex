...
- [Dashboard tutorial]({getting_started.dashboard_tutorial.path}) — a data app with tables, forms, and state.
- [Chatapp tutorial]({getting_started.chatapp_tutorial.path}) — streaming AI responses end-to-end.
- [Open-source templates]({getting_started.open_source_templates.path}) — full apps to fork.
```

```md alert info
# Go deeper →

- [Vars]({vars.base_vars.path}) and [var operations]({vars.var_operations.path}) — the full API.
- [Events]({events.events_overview.path}) and [pages]({pages.overview.path}) — routing, triggers, handlers.
- [How Reflex works]({advanced_onboarding.how_reflex_works.path}) — what runs where, and why.
```
