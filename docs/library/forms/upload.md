...
def upload_component():
    return rx.vstack(
        ...
        rx.foreach(State.uploaded_files, lambda f: rx.image(src=rx.get_upload_url(f), alt='Uploaded file preview')),
    )
```
...
def upload_component():
    return rx.vstack(
        ...
        rx.foreach(
            ...
            lambda filename: rx.image(src=rx.get_upload_url(filename), alt='Uploaded file preview'),
        ),
    )
```
...
```python demo box
rx.image(src="https://web.reflex-assets.dev/other/upload.webp", alt='Multiple file upload component example')
```
...
def index():
    ...
    return rx.vstack(
        ...
        rx.foreach(State.img, lambda img: rx.image(src=rx.get_upload_url(img), alt='Uploaded file preview')),
        padding="5em",
    )
```
