...
<circle cx="8" cy="8" r="4" fill="#56BA9F"/>
        ...
        rx.text(text, class_name="text-xs font-medium text-muted-foreground"),
        class_name=f"flex h-5 p-[0rem_0.375rem_0rem_0.125rem] items-center gap-0.5 rounded-[0.375rem] border border-border bg-background {class_name}",
    )


def purple_gradient_circle():
    return rx.box(
        class_name="w-80 h-80 flex-shrink-0 rounded-[20rem] bg-[radial-gradient(50%_50%_at_50%_50%,_var(--accent)_0%,_rgba(26,27,29,0.00)_100%)] absolute bottom-[-15.5rem] left-1/2 transform -translate-x-1/2 z-[-1]",
    )
...
def terminal_box():
    return rx.box(
        ...
        rx.box(
            ...
            rx.text(
                "terminal",
                class_name="text-muted-foreground text-xs font-medium text-center leading-4",
            ),
        ...
        rx.box(
            rx.text(
                "$",
                class_name="text-subtle-foreground text-xs font-medium text-center leading-4 font-['JetBrains_Mono'] mr-4 self-baseline",
            ),
            rx.box(
                rx.box(
                    rx.text(
                        "",
                        class_name="text-foreground text-xs font-medium text-center leading-4 font-['JetBrains_Mono'] reflex-deploy mr-0.5",
                    ),
                    # Square
                    rx.box(
                        class_name="w-2 h-4 border-2 border-subtle-foreground typing-square opacity-100 transition-all duration-200 ease-out"
                    ),
                    class_name="flex flex-row",
                ),
                rx.text(
                    "Deploying...",
                    class_name="text-subtle-foreground text-xs font-medium text-center leading-4 font-['JetBrains_Mono'] deploying-text self-start hidden",
                ),
        ...
        class_name="flex w-[20rem] h-[5rem] flex-col flex-shrink-0 rounded-xl rounded-b-none border-t-[1px] border-r-[1px] border-l-[1px] border-border-subtle bg-[light-dark(rgba(249, 249, 251, 0.48), rgba(26,27,29,0.48))] backdrop-filter backdrop-blur-[10px] terminal-box transition-all duration-[550ms] ease-in-out z-[5] origin-bottom translate-y-0 absolute bottom-0 overflow-hidden",
    )
...
def deploy_box():
    return rx.box(
        rx.box(
            ...
            rx.box(
                rx.text("My App", class_name="text-sm font-medium text-foreground"),
                rx.text(
                    "my-app.reflex.run",
                    class_name="text-sm font-medium text-subtle-foreground",
                ),
                class_name="flex flex-col",
            ),
        ...
        purple_gradient_circle(),
        class_name="flex w-[20rem] h-[4.5rem] flex-row flex-shrink-0 rounded-xl rounded-b-none border-t-[1px] border-r-[1px] border-l-[1px] border-border-subtle bg-[light-dark(rgba(249, 249, 251, 0.48), rgba(26,27,29,0.48))] backdrop-filter backdrop-blur-[10px] deploy-box transition-all duration-[550ms] ease-in-out z-[6] origin-bottom absolute items-center p-[1rem_1.625rem_1rem_1.25rem] overflow-hidden",
    )
