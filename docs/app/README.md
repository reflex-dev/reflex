...
Breadcrumbs and canonical URLs use `deploy_url` and `frontend_path` from the app config. Builds default to `https://reflex.dev`. Staging builds must set `REFLEX_DEPLOY_URL` to their public origin, without `/docs`; `frontend_path` adds that prefix once. Publish the generated sitemap and HTML metadata unchanged so canonical and social URLs stay aligned. A runtime sitemap-only rewrite does not update the HTML metadata.
