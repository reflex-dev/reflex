    ...
    class UploadState(rx.State):
        ...
        async def handle_upload_stream(self, chunk_iter: rx.UploadChunkIterator):
            ...
            async for chunk in chunk_iter:
                buf = file_buffers.get(chunk.filename)
                if buf is None:
                    buf = bytearray()
                    file_buffers[chunk.filename] = buf

                end = chunk.offset + len(chunk.data)
                if end > len(buf):
                    buf.extend(b"\x00" * (end - len(buf)))
                buf[chunk.offset : end] = chunk.data
            ...
            for filename, buf in file_buffers.items():
            ...
            async with self:
                self.stream_completed_files = sorted(file_buffers)
...
async def test_upload_chunk_file(tmp_path, upload_file: AppHarness, driver: WebDriver):
    ...
    assert [Path(record.split(":")[0]).name for record in chunk_records[:2]] == [
        "stream1.txt",
        "stream2.txt",
    ]
