...
#: The step every generated workflow installs uv with, and the placeholders one
#: of which has to follow it so the configured pins reach it.
...
def _uv_steps_without_a_checkout(rendered: str) -> list[dict]:
    """Return the setup-uv steps of every job that does not check the repo out.

    Args:
        rendered: A rendered workflow.

    Returns:
        The ``uses: astral-sh/setup-uv`` step mappings, in job order.
    """
    steps: list[dict] = []
    for job in yaml.safe_load(rendered)["jobs"].values():
        job_steps = job.get("steps") or []
        if any("actions/checkout@" in step.get("uses", "") for step in job_steps):
            continue
        steps += [
            step for step in job_steps if "astral-sh/setup-uv@" in step.get("uses", "")
        ]
    return steps


def test_render_quiets_setup_uv_where_there_is_no_checkout(config: Config) -> None:
    """A job with an empty workspace must not warn about it once per release.
    ...
    publish job) trips both, for a cache it has no dependencies to fill.
    """
    found = 0
    for name in CORE_WORKFLOWS:
        for step in _uv_steps_without_a_checkout(render(name, config)):
            found += 1
            assert step["with"]["enable-cache"] == "false", name
            assert step["with"]["ignore-empty-workdir"] == "true", name
...
def test_render_omits_the_block_when_nothing_is_pinned(repo: Path) -> None:
    ...
    quiet = _uv_steps_without_a_checkout(rendered)
    assert sum(1 for index in steps if lines[index + 1].strip() == "with:") == len(
        quiet
    )
    for step in quiet:
        assert set(step["with"]) == {"enable-cache", "ignore-empty-workdir"}
    assert "\n\n\n" not in rendered
