...
async def test_create(
    ...
    deployment_id = await client.deployments.create(
        APP_ID,
        backend=backend,
    ...
    assert form.pop("reflex_sdk_version")
...
async def test_wait_raises_on_failure(client: AsyncReflexCloud, mock_api: MockAPI):
    ...
    with pytest.raises(
        ...
        await client.deployments.wait(uuid.UUID(FIRST_ID), poll_interval=0)
...
async def test_wait_checks_the_recorded_state_periodically(
    ...
    monkeypatch.setattr(
        "reflex_sdk._async.resources.deployments._REPORT_EVERY_POLLS", 3
    )
...
async def test_set_description(client: AsyncReflexCloud, mock_api: MockAPI):
    ...
    await client.deployments.set_description(APP_ID, FIRST_ID, "release 2")
    assert json_body(mock_api.requests[0]) == {"description": "release 2"}


async def test_regions_and_vm_types_need_no_token(mock_api: MockAPI):
    mock_api.add(
        "GET",
    ...
    async with AsyncReflexCloud(transport=AsyncMockTransport(mock_api)) as client:
        ...
        assert await client.deployments.vm_types() == [
            MachineSize(
                id="c1m1",
                name="Single CPU Small",
                cpu=1.0,
