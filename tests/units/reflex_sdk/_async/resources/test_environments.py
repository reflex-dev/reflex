...
import pytest
from reflex_sdk import APIResponseValidationError, AsyncReflexCloud
...
async def test_list(client: AsyncReflexCloud, mock_api: MockAPI):
    ...
    dev, production = await client.apps.environments.list(APP_ID)
...
async def test_enable(client: AsyncReflexCloud, mock_api: MockAPI):
    ...
    assert await client.apps.environments.enable(APP_ID) == EnvironmentsEnabled(
...
async def test_create(client: AsyncReflexCloud, mock_api: MockAPI):
    ...
    assert await client.apps.environments.create(
...
async def test_update_sends_only_the_changed_settings(
    ...
    await client.apps.environments.update(APP_ID, DEV_ID, requires_approval=False)
...
async def test_reorder(client: AsyncReflexCloud, mock_api: MockAPI):
    ...
    await client.apps.environments.reorder(APP_ID, [uuid.UUID(APP_ID), DEV_ID])
    assert json_body(mock_api.requests[0]) == {
...
async def test_promote(client: AsyncReflexCloud, mock_api: MockAPI):
    ...
    assert await client.apps.environments.promote(
        APP_ID,
        APP_ID,
...
async def test_promote_sends_an_object_without_options(
    ...
    await client.apps.environments.promote(APP_ID, APP_ID)
    assert json_body(mock_api.requests[0]) == {}
...
async def test_copy_missing_secrets(
    ...
    assert await client.apps.environments.copy_missing_secrets(APP_ID, APP_ID) == copied
...
async def test_delete(client: AsyncReflexCloud, mock_api: MockAPI):
    ...
    assert await client.apps.environments.delete(APP_ID, DEV_ID) is None
