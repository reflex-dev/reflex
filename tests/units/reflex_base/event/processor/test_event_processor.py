...
async def _sibling_superseding_handler(value: str):
    """Record a superseding sibling invocation."""
    await asyncio.sleep(0)
    _CALL_LOG.append({"value": value})


_sibling_superseding_handler._reflex_supersedes = True  # type: ignore[attr-defined]
...
sibling_superseding_event = EventHandler(fn=_sibling_superseding_handler)
...
async def test_sibling_superseding_events_use_latest_invocation(
    processor: EventProcessor,
    token: str,
):
    """A later superseding sibling cancels an earlier queued invocation."""
    processor.configure()
    async with processor as ep:
        first = await ep.enqueue(
            token,
            Event.from_event_type(sibling_superseding_event("first"))[0],
        )
        second = await ep.enqueue(
            token,
            Event.from_event_type(sibling_superseding_event("second"))[0],
        )
        await asyncio.wait_for(second.wait_all(), timeout=1)

    assert first.cancelled()
    assert _CALL_LOG == [{"value": "second"}]
