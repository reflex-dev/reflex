...
def test_single_hook_no_useState() -> None:
    ...
    hook = _hook(cs)
    assert hook == (
        'const [counterRxClientState, setCounter] = useClientState(0, "counter", true)'
    )
...
def test_omitted_default_emits_valid_javascript() -> None:
    ...
    """
    cs = client_state(name="counter")
    hook = _hook(cs)
    assert "(," not in hook
    assert hook == (
        "const [counterRxClientState, setCounter] = "
        'useClientState(undefined, "counter", true)'
    )


def test_unnamed_var_is_scoped_not_global() -> None:
    """An unnamed var still gets a name, but no flag escaping it to the root."""
    cs = client_state(default=False)
    assert _hook(cs).endswith(f'useClientState(false, "{cs._state_name}")')
    assert not cs._is_global


def test_named_var_is_global() -> None:
    """Naming a var is what makes it app-wide and backend-addressable."""
    cs = client_state(False, name="shared")
    assert _hook(cs).endswith('useClientState(false, "shared", true)')
    assert cs._is_global


def test_hook_imports_use_client_state() -> None:
    """The hook carries the useClientState import."""
    imports = dict(cs_imports := client_state(0, name="x")._var_data.imports)  # pyright: ignore [reportOptionalMemberAccess]
...
def test_provider_app_wrap_declared(name: str | None) -> None:
    """The provider is requested either way; the hook always uses context."""
...
def test_value_is_marked_identifier() -> None:
    """``value`` renders the marked local binding."""
    cs = client_state(0, name="counter")
...
def test_set_functional_updater_is_typed(default: Any, fn: Any, expected: str) -> None:
    ...
    cs = client_state(default, name="x")
...
def test_underscore_named_var_is_not_mistaken_for_event_arg() -> None:
    ...
    private = client_state("", name="_private")
    other = client_state("", name="other")
...
def test_reserved_words_are_safe(reserved: str) -> None:
    ...
    cs = client_state(1, name=reserved)
    hook = _hook(cs)
...
def test_camel_case_names_get_distinct_setters() -> None:
    ...
    assert (
        client_state(name="myVar")._setter_name
        != client_state(name="myvar")._setter_name
    )
...
def test_var_name_rejects_var() -> None:
    ...
    with pytest.raises(ValueError, match="not a Var"):
        client_state(name=Var("dynamic"))  # pyright: ignore [reportArgumentType]
...
def test_name_addressed_paths_require_global(accessor: str) -> None:
    ...
    cs = client_state(default=0)
    with pytest.raises(ValueError, match="scoped to the component tree"):
        if accessor == "push":
            cs.push(1)
...
def test_var_renders_as_null() -> None:
    ...
    assert str(client_state(0, name="x")) == "null"
...
def test_acceptance_throttle_controlled_input_compiles() -> None:
    ...
    def debounce_controlled_input(
        ...
        lc_var = rx.client_state()
        lc_last_var = rx.client_state()
        return rx.el.input(
            rest,
            rx.cond(
                value != lc_var.value,
    ...
    # Distinct bindings, and neither escapes to the root scope (a named var
    # would pass a trailing `true`).
    assert len(set(declarations)) == 2
    assert all(not line.rstrip(";").endswith("true)") for line in declarations)
...
def test_set_binds_memo_prop_var_without_declaring_an_arg() -> None:
    ...
    comp(value="x")
    cs = rx.client_state("", name="target")
...
def test_hash_distinguishes_vars() -> None:
    ...
    a = client_state(0, name="a")
    b = client_state(0, name="b")
...
def test_var_name_rejects_non_string() -> None:
    ...
    with pytest.raises(ValueError, match="must be a string"):
        client_state(name=5)  # pyright: ignore [reportArgumentType]
...
def test_each_construction_gets_its_own_name() -> None:
    """A plain helper called N times yields N independent states.

    ``rx.client_state()`` runs once per call at compile time, so a helper
    function that constructs one gives every call site its own slot -- no memo,
    no keys, no configuration.
    """
    names = [client_state(0)._state_name for _ in range(3)]
    assert len(set(names)) == 3


def test_all_consumers_of_one_var_share_the_name() -> None:
    """Every consumer emits the same hook, so an auto-memo split still shares.

    This is the property the redesign exists for: reading and writing a var
    compile to separate memo modules, and they must resolve the same slot.
    """
    cs = client_state(0)
    hooks = {
        _hook(cs),
        *(
            hook
            for accessor in (cs.value, cs.set, cs.set(1))
            for hook in (accessor._get_all_var_data() or VarData()).hooks
        ),
    }
    assert len(hooks) == 1, f"expected one shared hook, got {hooks}"


def test_prefix_customizes_the_generated_name() -> None:
    """A prefix keeps the compiled javascript readable for internal use."""
    cs = client_state(0, prefix="counter")
    assert cs._state_name.startswith("counter")
    assert _hook(cs).startswith("const [counter")


def test_prefix_is_ignored_when_named() -> None:
    """An explicit name wins; the prefix only shapes generated names."""
    cs = client_state(0, name="explicit", prefix="ignored")
    assert cs._state_name == "explicit"


def test_generated_names_stay_unique_across_prefixes() -> None:
    """One shared counter, so mixing prefixes can never collide."""
    names = [
        client_state(0, prefix="a")._state_name,
        client_state(0, prefix="b")._state_name,
        client_state(0)._state_name,
    ]
    assert len(set(names)) == 3


@pytest.mark.parametrize("bad", ["1bad", "my-prefix", "", "a b"])
def test_prefix_must_be_an_identifier(bad: str) -> None:
