...
from reflex.compiler.plugins.memoize import (
...
def test_memoize_wrapper_emits_declaration_and_call_site() -> None:
    """Memoizable component emits wrapper declaration + wraps at call-site."""
    ...
    # Wrapper declaration in page custom code.
    assert f"const {wrapper_tag} = memo(({{ children }}) => children)" in (
        page_ctx.output_code or ""
    )
    # Wrapper wraps the original component at its call-site.
    assert f"jsx({wrapper_tag}," in (page_ctx.output_code or "")
    # `{memo}` from react is imported.
    assert "memo" in (page_ctx.output_code or "")


def test_memoize_wrapper_deduped_across_repeated_subtrees() -> None:
    """Two identical memoizable call-sites collapse to one wrapper declaration."""
    ...
    assert len(ctx.memoize_wrappers) == 1
    # Declaration appears exactly once in module code.
    wrapper_tag = next(iter(ctx.memoize_wrappers))
    assert sum(f"const {wrapper_tag} = memo" in k for k in page_ctx.module_code) == 1
...
def test_memowrappercomponent_is_not_itself_memoized() -> None:
    """The wrapper component instance itself has no state deps and is skipped."""
    # Sanity check: calling _should_memoize on a MemoWrapperComponent instance
    # returns False so the plugin won't recursively wrap its own wrappers.
    from reflex.compiler.plugins.memoize import _get_memo_wrapper_class

    wrapper = _get_memo_wrapper_class("MyTag")._create(children=[])
...
def test_memo_wrapper_component_renders_as_own_tag() -> None:
    """A MemoWrapperComponent renders as <Tag>{children}</Tag> with correct tag."""
    from reflex.compiler.plugins.memoize import _get_memo_wrapper_class

    cls = _get_memo_wrapper_class("MyWrapper_abc")
    wrapper = cls._create(children=[Plain.create()])
    assert isinstance(wrapper, MemoWrapperComponent)
    assert wrapper.tag == "MyWrapper_abc"
    # Wrapper declares its own custom code line.
    assert (
        wrapper._get_custom_code()
        == "const MyWrapper_abc = memo(({ children }) => children)"
    )
    # Wrapper imports `memo` from react.
    assert "react" in wrapper._get_imports()
...
def test_shared_subtree_across_pages_uses_same_tag() -> None:
    ...
    for route in ("/a", "/b"):
        ...
        assert f"const {tag} = memo" in output
