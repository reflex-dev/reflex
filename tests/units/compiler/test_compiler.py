...
def test_embed_manifest_path_for(route: str, expected_path: str):
    """Reflex routes translate to React Router 7 path strings."""
    assert compiler._embed_manifest_path_for(route) == expected_path


def test_compile_embed_manifest(mocker: MockerFixture, tmp_path: Path):
    """``compile_embed_manifest`` writes one entry per route with matching stems."""
    ...
    output_path, code = compiler.compile_embed_manifest(routes)

    assert output_path == str(
        tmp_path / constants.Dirs.PAGES / "__reflex_embed_manifest.js"
    )
    assert code.startswith("// Generated by reflex")
    assert "export default [" in code

    expected_paths = ["", "about", "users/:id", "posts/:slug?", "docs/*", "*"]
    for path in expected_paths:
        assert f'path: "{path}"' in code

    for route in routes:
        specifier = utils.get_page_import_specifier(route)
        assert f'load: () => import("{specifier}")' in code
