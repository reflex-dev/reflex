# Generated from tests/units/reflex_build_sdk/_async/resources/test_usage.py by packages/reflex-build-sdk/scripts/unasync.py. Do not edit.
...
def test_balance(client: ReflexBuild, mock_api: MockAPI):
    ...
    assert client.usage.balance() == UsageBalance(
        has_allowance=True,
        used_pct="37.50",
        remaining_pct="62.50",
        refresh_eligible=True,
...
def test_history_follows_the_cursor_header(client: ReflexBuild, mock_api: MockAPI):
    ...
    entries = [
        entry
        for entry in client.usage.history(
