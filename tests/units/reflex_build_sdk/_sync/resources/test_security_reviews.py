# Generated from tests/units/reflex_build_sdk/_async/resources/test_security_reviews.py by packages/reflex-build-sdk/scripts/unasync.py. Do not edit.
...
def test_submit(client: ReflexCloud, mock_api: MockAPI, tmp_path: Path):
    ...
    assert client.security_reviews.submit(archive) == JOB_ID
...
def test_get(client: ReflexCloud, mock_api: MockAPI):
    ...
    assert client.security_reviews.get(JOB_ID) == SecurityReviewJob(
        job_id=JOB_ID,
        status="complete",
...
def test_wait(client: ReflexCloud, mock_api: MockAPI):
    ...
    result = client.security_reviews.wait(JOB_ID, poll_interval=0)
