...
import pandas as pd
# PATCHED FOR OFFLINE TESTING (see NOTES.md): the egress proxy in this test
# environment blocks fc.yahoo.com / query*.finance.yahoo.com with 403, so
# yf.download() returns an empty DataFrame and the demo has nothing to show.
# _offline_download() produces a deterministic DataFrame with EXACTLY the shape
# yfinance returns (MultiIndex columns (ticker, field), DatetimeIndex named
# "Date"); nothing else in the app is changed.
import math as _math

import numpy as _np
import yfinance as yf  # noqa: F401  (kept so the dependency is still exercised)


def _offline_download(tickers, start, end, group_by="ticker"):
    """Deterministic stand-in for yfinance.download() with the same frame shape."""
    ...
    for t_i, ticker in enumerate(tickers):
        ...
        for d_i in range(len(dates)):
            wave = _math.sin((d_i + t_i * 7) / 9.0)
            o = base + 10.0 * wave + 0.15 * d_i
            c = o + 2.0 * _math.cos((d_i + t_i * 3) / 5.0)
            h = max(o, c) + 1.5 + abs(wave)
            lo = min(o, c) - 1.5 - abs(wave)
            opens.append(o); highs.append(h); lows.append(lo); closes.append(c)
            vols.append(1_000_000 + 25_000 * ((d_i * 13 + t_i * 7) % 40))
        frames[ticker] = pd.DataFrame(
            {
                "Open": _np.array(opens),
                "High": _np.array(highs),
                "Low": _np.array(lows),
                "Close": _np.array(closes),
                "Volume": _np.array(vols),
            },
            index=dates,
        )
...
class State(rx.State):
    ...
    def fetch_stock_data(self):
        ...
        # PATCHED FOR OFFLINE TESTING: yfinance is blocked by the egress proxy here.
        self.data = _offline_download(
            companies,
            start=start_date,
