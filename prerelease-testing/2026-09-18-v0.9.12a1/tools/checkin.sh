...
for j in $WF/*/journal.jsonl; do
  ...
  jq -r 'select(.type=="result") | "  done  " + (.label // "?") + "  tests=" + ((.result.tests // []) | length | tostring) + " issues=" + ((.result.issues // []) | length | tostring) + " verdicts=" + ((.result.verdicts // []) | length | tostring)' $j 2>/dev/null
  comm -23 <(jq -r 'select(.type=="started") | .label' $j | sort) <(jq -r 'select(.type=="result" or .type=="error" or .type=="skipped") | .label' $j | sort) | sed 's/^/  running  /'
done
