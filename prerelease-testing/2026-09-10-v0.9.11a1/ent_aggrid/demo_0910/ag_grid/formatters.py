...
# PATCHED FOR TESTING (see NOTES.md): the shipped call was
#     dynamic.bundle_library("$/utils/components")
# which is a stale pre-0.9.8 path -- since reflex 0.9.8 @rx.memo components compile
# under $/app_components/<module path>. compile_app() also calls
# reset_bundled_libraries() AFTER importing the app module, so an import-time-only
# bundle_library() call is wiped before pages are compiled: re-bundle at page eval time.
...
def formatter_page():
    """AG Grid with custom formatters."""
    _bundle_renderer_libraries()  # PATCHED FOR TESTING: see note at top of file
    return rx.vstack(
        rx.box(
