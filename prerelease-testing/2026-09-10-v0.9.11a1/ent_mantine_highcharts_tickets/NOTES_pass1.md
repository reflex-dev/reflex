> **Superseded in part by `NOTES.md` (pass 2).** Pass 2 drove the same demos as a user in
> dev *and* prod and found that the console error recorded here as FINDING-036 actually
> leaves the whole tickets UI inert — no event is ever dispatched. See `NOTES.md` ISSUE 1.
> The structural comparison below and the artifacts it cites (`out/`, `logs/`,
> `drive_ent.py`, `runpair.sh`) still stand.
