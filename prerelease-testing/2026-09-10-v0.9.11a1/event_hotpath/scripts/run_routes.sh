...
sleep 30
$A/scripts/wait_up.sh "http://localhost:$fp$fpath/" 420 || { tail -30 $A/$LOGDIR/routes_$label.log; }
grep -n -E 'Compiling|Reflex 0' $A/$LOGDIR/routes_$label.log | head -2
cd $A/scripts && NO_PROXY=localhost,127.0.0.1 no_proxy=localhost,127.0.0.1 timeout 700 $SB/envs/driver/bin/python pw_routes.py \
  --url http://localhost:$fp --frontend-path "$fpath" --out $A/$LOGDIR/pw_routes_$label --label $label 2>&1 \
