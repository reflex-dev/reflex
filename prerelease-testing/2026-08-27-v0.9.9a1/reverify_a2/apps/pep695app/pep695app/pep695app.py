...
_logger = logging.getLogger("pep695app")
...
class AliasState(rx.State):
    ...
    @rx.event
    async def upload_probe(self, files: list[rx.UploadFile]):
        """Buffered upload handler for the item-3 sanitizer live test.
        ...
        for file in files:
            ...
            self.saved_names = [*self.saved_names, str(file.name)]
    ...
    @rx.event
    def do_log(self):
        """Emit worker-side log records (item 8: full-logging capture)."""
        _logger.info("pep695app worker log hierarchy-marker key=%s", self.key)
        logging.getLogger("reflex_base").info(
            "reflex_base worker hierarchy-marker %s", self.name
        )
    ...
    return rx.container(
        rx.vstack(
            ...
            rx.hstack(
                ...
                rx.button("log", on_click=AliasState.do_log, id="btn-log"),
            ),
            ...
            rx.select(
                ...
                on_change=AliasState.choose_key,
            ),
            rx.input(
                placeholder="rename",
