...
def install_frontend_packages(packages: set[str]):
    ...
    if not hasattr(install_frontend_packages, "customPackagesCache"):
        setattr(install_frontend_packages, "customPackagesCache", set())
    
    currPackagesToInstall = list()
    for package in packages:
        if not package in install_frontend_packages.customPackagesCache:
            currPackagesToInstall.append(package)
            install_frontend_packages.customPackagesCache.add(package)

    # Install custom packages defined in frontend_packages
    if len(currPackagesToInstall) > 0:
        process = processes.new_process(
            [get_install_package_manager(), "add", *currPackagesToInstall],
            cwd=constants.Dirs.WEB,
            shell=constants.IS_WINDOWS,
        )
...
def validate_frontend_dependencies(init=True):
    ...
    """
    # TODO: Do a better job at validating frontend dependencies
    if not init:
...
def initialize_frontend_dependencies():
    ...
    if constants.IS_WINDOWS:
        ...
        if not check_bun_version():
            # install_bun()
            # We've noted some instablility in bun and is under investigation
            install_node()
...
def prompt_for_template() -> constants.Templates.Kind:
    ...
    console.print("blank (https://blank-template.nextpy.run) - A minimal template.")
    console.print(
        "sidebar (https://sidebar-template.nextpy.run) - A template with a sidebar to navigate pages."
    )
    console.print("")
