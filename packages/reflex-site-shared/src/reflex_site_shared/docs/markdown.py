"""Render shared documentation with the Reflex docgen Markdown pipeline."""

# Transformer overrides inherit their contracts from DocumentTransformer.
...
def get_markdown_toc(text: str) -> list[tuple[int, str]]:
    ...
    Returns:
        Heading ``(level, text)`` tuples in document order.
    """
    doc = parse_document(text)
    return [
        (heading.level, _spans_to_plaintext(heading.children))
