...
class ReflexPyiBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict[str, Any]) -> None:
        ...
        try:
            from reflex.utils.pyi_generator import PyiGenerator  # noqa: F401
        except ImportError:
            # reflex is not installed — skip pyi generation.
        ...
        subprocess.run(
            [sys.executable, "-m", "reflex.utils.pyi_generator", src_dir.name],
            cwd=src_dir.parent,
            check=True,
        )
        self._marker().touch()
