...
| `--min-runs`, `--max-runs`, `--min-time` | 10, 30, 30 s | The automatic run count (below). |
...
times out is reported with its status and does not stop the run.
...
`--keep`), `cache_dir` (persistent per subject and benchmark id, shared by the
parameter sets, for `setup_cache` results; the hook decides what to reuse),
...
and the remaining teardown hooks run on a fresh one, so `conclude` should kill
whatever `sample` started. `reflex_bench.registry.Instance` exposes the hooks as
zero-argument methods for callers other than the scheduler.
...
- the change is `median(head) / median(base) - 1`, with a percentile bootstrap
  confidence interval (seeded, 10,000 resamples);
...
cache/<subject>/<benchmark id>/
```

The machine profile id is `<os>-<arch>-<cpu model>-py<major.minor>`, for example
`linux-x86_64-ryzen-9-7950x-py3.12`; `REFLEX_BENCH_PROFILE` overrides it. Results
from different profiles are never compared.
