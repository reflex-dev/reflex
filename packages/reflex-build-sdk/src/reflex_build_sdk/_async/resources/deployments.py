...
from reflex_build_sdk._base import path_segment, sdk_version
...
class AsyncDeployments:
    ...
    async def create(
        ...
        *,
        ...
        form = {
            ...
            "reflex_build_sdk_version": sdk_version(),
    ...
    async def _upload(
        ...
        for _ in range(UPLOAD_ATTEMPTS - 1):
            ...
            try:
                await uploader.upload(_upload_targets(archives, sizes, reservation))
    ...
    async def status(self, deployment_id: uuid.UUID | str) -> str:
    ...
    async def report(self, deployment_id: uuid.UUID | str) -> DeploymentReport:
    ...
    async def build_logs(self, deployment_id: uuid.UUID | str) -> str:
    ...
    async def wait(
        ...
        *,
        ...
        polls = 0
        while True:
            message = await self.status(deployment_uuid)
            ...
            if outcome is not None or stale or polls % _REPORT_EVERY_POLLS == 0:
                report = await self.report(deployment_uuid)
                if outcome is None:
    ...
    async def vm_types(self) -> list[MachineSize]:
