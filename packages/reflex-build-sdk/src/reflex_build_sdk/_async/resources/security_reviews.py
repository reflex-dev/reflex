...
from reflex_build_sdk._deploy import AsyncArchiveUploader, UploadTarget
...
class AsyncSecurityReviews:
    ...
    async def submit(self, archive: str | os.PathLike[str]) -> str:
        ...
        upload = await self._client._request(
            "POST",
            "agents/security-review/jobs/upload-url",
            _UploadUrl,
            json={"content_length": size, "content_type": "application/zip"},
        )
        await AsyncArchiveUploader(
            self._client._transport, None, self._client._timeout
        ).upload([(path, size, UploadTarget(url=upload.url, headers=upload.headers))])
        job = await self._client._request(
            "POST",
    ...
    async def get(self, job_id: str) -> SecurityReviewJob:
