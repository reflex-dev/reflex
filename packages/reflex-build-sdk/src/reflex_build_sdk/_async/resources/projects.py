...
from reflex_build_sdk._errors import NotFoundError
...
class AsyncTeams:
    ...
    async def list(self, project_id: uuid.UUID | str) -> TeamGrants:
    ...
    async def revoke(
        self, project_id: uuid.UUID | str, team_id: uuid.UUID | str
    ) -> str:
...
class AsyncProjects:
    ...
    async def rename(self, project_id: uuid.UUID | str, name: str) -> None:
        """Rename a project.
    ...
    async def delete(self, project_id: uuid.UUID | str) -> None:
        ...
        """
        await self._client._request(
            "DELETE", f"project/{path_segment(project_id)}", None
        )
