...
APP_DEV_WORKFLOW = "deploy_dev.yml"

#: Generated workflows that a repository may stop needing.
OPTIONAL_WORKFLOWS = (INTERNAL_WORKFLOW, APP_DEV_WORKFLOW)
...
def managed_workflows(config: Config) -> tuple[str, ...]:
    ...
    if config.app:
        return (
            *CORE_WORKFLOWS,
            *((APP_DEV_WORKFLOW,) if config.app.dev_schedule else ()),
        )
...
def render(name: str, config: Config) -> str:
    ...
    if config.app:
        substitutions = {
            ...
            "@@STAGING_ENVIRONMENT@@": config.app.staging_environment,
            "@@DEV_ENVIRONMENT@@": config.app.dev_environment,
            "@@DEV_SCHEDULE@@": config.app.dev_schedule,
            "@@ALLOW_SELF_REVIEW@@": "true" if config.allow_self_review else "false",
        }
        text = (TEMPLATE_DIR / "app" / name).read_text(encoding="utf-8")
        for placeholder, value in substitutions.items():
            text = text.replace(placeholder, value)
        if re.search(r"@@[A-Z_]+@@", text):
            fail(f"unsubstituted app workflow placeholder in {name}")
        return text
...
def generated_workflow_names() -> set[str]:
    ...
    for target in TEMPLATE_DIR.rglob("*.yml"):
        text = target.read_text(encoding="utf-8")
        match = _WORKFLOW_NAME_RE.search(text)
        if match is None:
            fail(f"the {target.name} template declares no top-level name")
...
def sync(config: Config, check: bool = False, force: bool = False) -> None:
    ...
    if config.app:
        for workflow, inputs in (
            (config.app.build_workflow, {"ref", "version", "source-revision"}),
            (config.app.deploy_workflow, {"ref", "version", "environment"}),
        ):
            target = config.root / WORKFLOW_DIR / workflow
            if workflow in GENERATED_WORKFLOWS or not target.is_file():
                fail(
    ...
    for name in OPTIONAL_WORKFLOWS:
        ...
        if check:
            stale.append(name)
            echo(f"{WORKFLOW_DIR}/{name} is obsolete for the current configuration")
            continue
        target.unlink()
        echo(f"removed {WORKFLOW_DIR}/{name} (no longer configured)")
...
def init(root: Path, pin: str | None, force: bool) -> None:
    ...
    if "app" not in load_pyproject(pyproject).get("tool", {}).get(TOOL_TABLE, {}):
...
def print_checklist(config: Config) -> None:
    ...
    if config.app:
        echo(
            f"Configure required reviewers on {config.app.production_environment!r}, "
            f"protect {config.main_branch!r}, and enable Actions to create PRs. "
            "Build/deploy hooks own their credentials and must report deployment "
            "completion before returning success. Pin cli-command to a published "
            "reflex-release version with app support."
        )
        return
