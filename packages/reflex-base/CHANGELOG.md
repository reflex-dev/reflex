...
- Add the opt-in `frontend_lazy_bundled_libraries` config setting to defer optional dynamic-component libraries until first use and reduce the number of initial library requests. React and the shared runtime remain immediately available; the setting does not guarantee fewer total JavaScript bytes for every application. ([#7078](https://github.com/reflex-dev/reflex/issues/7078))
