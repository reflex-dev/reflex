...
def upload_js_template(extra_headers: Iterable[tuple[str, str]]) -> str:
    """Template for upload.js.

    Args:
        extra_headers: Extra headers to include in the upload request.

    Returns:
        Rendered upload.js content as string.
    """
    add_extra_headers = "\n".join(
        [
            f"    xhr.setRequestHeader({json.dumps(name)}, {json.dumps(value)});"
            for name, value in extra_headers
        ]
    )
    return rf"""import JSON5 from "json5";
