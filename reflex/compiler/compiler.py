...
from reflex.compiler.plugins import CompileContext, CompilerHooks, default_page_plugins
...
def _set_progress_total(
    ...
    if task not in progress.tasks:
        return

    previous_total = progress.tasks[task]["total"]
    progress.tasks[task]["total"] = total
    progress.total += total - previous_total
...
def _compile_page_from_app(
    ...
    *,
    ...
    """Evaluate a single app page for compatibility call sites.

    Args:
        app: The app containing the page definition.
        route: The route to evaluate.
        save_page: Whether to store the evaluated component on ``app._pages``.

    Returns:
        The evaluated page component.
    """
    ...
    component = compile_unevaluated_page(
        route,
        app._unevaluated_pages[route],
        app.style,
        app.theme,
    )

    if len(all_base_state_classes) > n_states_before:
        app._stateful_pages[route] = None

    app._check_routes_conflict(route)
    if save_page:
        app._pages[route] = component
    return component
...
def _resolve_app_wrap_components(
    ...
    Returns:
        The merged app-wrap component mapping.
    """
    config = get_config()
...
def _build_app_root(app_wrappers: dict[tuple[int, str], Component]) -> Component:
    """Create the wrapped app root component from ordered wrappers.

    Returns:
        The wrapped app root component.
    """
...
def compile_app(
    ...
    *,
    ...
    app_root = _build_app_root(app_wrappers)
