...
class AppHarness:
    ...
    def get_full_state_name(self, path: list[str]) -> str:
        ...
        Returns:
            The full state name
        """
        # Default scheme like get_state_name; get_name() would apply the resolver.
        path = [RegistrationContext.default_state_name(State)] + [
            self.get_state_name(p) for p in path
        ]
        return ".".join(path)
