...
def get_app(reload: bool = False) -> ModuleType:
    ...
    app = __import__(module, fromlist=(constants.VAR_NAMES.APP,))
    if reload:
...
def initialize_app_directory(app_name: str, template: constants.TEMPLATE.KIND):
    ...
    path_ops.cp(
        os.path.join(constants.TEMPLATE.DIR.BASE, "apps", template.value), app_name
    )
    path_ops.mv(
        os.path.join(app_name, template.value + ".py"),
        os.path.join(app_name, app_name + constants.EXT.PY),
    )
    path_ops.cp(constants.TEMPLATE.DIR.ASSETS_TEMPLATE, constants.DIRS.APP_ASSETS)
...
def initialize_web_directory():
    ...
    path_ops.cp(constants.TEMPLATE.DIR.WEB_TEMPLATE, constants.DIRS.WEB)
...
def _compile_package_json():
    return templates.PACKAGE_JSON.render(
        scripts={
            "dev": constants.PACKAGE_JSON.COMMANDS.DEV,
            "export": constants.PACKAGE_JSON.COMMANDS.EXPORT,
            "export_sitemap": constants.PACKAGE_JSON.COMMANDS.EXPORT_SITEMAP,
            "prod": constants.PACKAGE_JSON.COMMANDS.PROD,
        },
        dependencies=constants.PACKAGE_JSON.DEPENDENCIES,
        dev_dependencies=constants.PACKAGE_JSON.DEV_DEPENDENCIES,
    )


def initialize_package_json():
    """Render and write in .web the package.json file."""
    output_path = constants.PACKAGE_JSON.PATH
