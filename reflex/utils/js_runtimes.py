    ...
    if not path_ops.samefile(bun_path, constants.Bun.DEFAULT_PATH):
        ...
        if bun_version < version.parse(constants.Bun.MIN_VERSION):
            logger.warning(
                ...
                f"Bun 1.4 creates new lockfiles at `lockfileVersion: 2`, which bun 1.3.x cannot parse, so a "
                f"[bold]reflex.lock/bun.lock[/bold] generated by bun 1.4 will fail to install with this bun. "
                f"An existing v1 lockfile is kept at v1, so already-initialized projects are unaffected.",
                extra={"rich": True},
            )
