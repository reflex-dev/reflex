...
**Component cache: built, measured on the real app, then REMOVED.** A construction cache (`Component._create` memoization of fully-static subtrees) was implemented and made byte-correct, but instrumenting it on the docs app showed it is **not worth keeping — only 1.1×** — and the measurement explains exactly why the §86 "72.7% duplicates" intuition does not translate to speedup:

- **Only 37% of constructions are cacheable, 23% hit.** Of 94,795 `_create` calls: 34,684 (37%) are fully static; the other 63% touch a state `Var`/event/handler — and *one* dynamic descendant poisons every ancestor. Hits land on 23% of all calls, and those are the **cheap static leaves** (text, plain divs), not the expensive state/event components. The "72.7% duplicates" counts *post-render structural* duplicates, including the dynamic ones the construction cache can never reuse.
- **Construction is only 19% of the compile.** Building the trees is 8.6s of a 46s compile; the other **81%** is render + memoization (`deepcopy` + structural hash, the slow part) + codegen + aggregation — none of which a construction cache touches. So a *perfect* construction cache caps at ~1.23×; catching the cheap 23% of the 19% yields **1.1×**.
- **Verdict.** The construction cache attacks the wrong 19%. Removed (module, flag, tests). The real cold-build lever is the **81%** — memoize `deepcopy`+hash is the fattest target (a pure transform, exactly what Rust would win — see §1), or caching rendered JS keyed by a structural hash. The disk cache (skip whole unchanged pages — all 100%) remains the high-value win and is where caching effort belongs.
