...
**Measured on the real docs app (417 pages, in-process at the `CompileContext` level, no npm).** Baseline cold compile: **54.6s** (101 pages reference state; only 3 *register new* state on eval). Disk cache, simulating the dev loop (edit one page → recompile only it, reuse the rest): **6.5s → 8.3×**, and it is correct by construction (it replays the prior compile's exact output + memo files, which are mutually consistent; reuse is per-process so each build is internally clean). The disk cache is the shippable win for the edit loop. **Component cache: a correctness bug, not just a speedup.** It cut the cold compile to 35.4s (1.54×) but the output was **not** byte-identical to baseline — and the cause is real, not measurement noise: a shared static subtree (the redirect `<head>` fragment: `<title>`+`<meta>`) is reused across the ~20 redirect pages, and the head-injection pass mutates it **in place** (appending children) on each page, so by render time the fragment is duplicated ~20×. The `own()` copy-on-write the cache relied on does **not** cover that mutation path. The deterministic hash is md5 (no randomness), so this is intrinsic to sharing live mutable instances across pages within one compile — the synthetic test was too simple to exercise a mutate-in-place path. **Conclusion: the component cache must not be enabled without `REFLEX_COMPILE_CACHE_VERIFY` (which catches the divergence and falls back), or — better — the compile pipeline must `own()` before every child mutation (a framework fix beyond the cache).** A separate observation from the spike: running *many* compiles in one process is non-deterministic (≈279/417 pages differ at identical length — auto-memo content hashes shift as cross-compile global state accumulates), so byte-compare verification is only meaningful per fresh process, not across in-process recompiles; this does not affect the disk cache (cross-process, one clean compile each).
