...
**Stage B — native construction, purity-gated (the perf win).** *Started: rx.el.* elements done.* Port the histogram-ranked types so the tree is built via the native registry (`make`, GIL-pure — Revision 2) instead of `Component.create`. The registry needs only `{tag, kind}` per component (+ a style mapping for radix); prop **ordering is a sort** by camelCased key (matching `format_props`), not a per-component field-order table — verified byte-identical. **Done & verified:** the `rx.el.*` element set — byte-identical across multi-prop/`id`→`ref`/nested/cond-child/foreach-child trees, ~12× construction speedup on ~12k nodes, GIL-pure (`experimental/reflex-compiler-core/stage_b.py`). **Next:** radix layout (`rx.box/text/flex` — needs JS tag + `add_style`→class mapping from the Python class) and native Cond/Iterable construction (read Var `_js_expr` at build) so they become build-native, not just codegen-native. **Exit per ported type:** §5.7 purity gate (GIL-released render + fallback 0) **and** byte-identical output. **Milestone exit:** the docs app compiles with the fallback count driven to a small *declared* allowlist (e.g. `markdown` may stay a leaf initially) and a measured per-page speedup. State/events remain Python leaves throughout.
...
**Implemented (flag-gated): persistent compile cache, Tier 1.** `reflex/compiler/page_cache.py` + `REFLEX_PERSISTENT_COMPILE_CACHE` (off by default). It computes a *complete* source fingerprint (every `.py`/`.md`/`.mdx` under the project root + the Reflex version) and, when unchanged since the last successful compile, skips the frontend compile and reuses the existing `.web` output — content-aware, where today's `_should_compile` is a manual nocompile-file check. Correctness: the fingerprint hashes all output-affecting source, so any edit (page, shared component, state, `rxconfig.py`) forces a full recompile — no stale output. Verified: 6 unit tests, no regression in the 185 compiler tests, and end-to-end on a real app (build1 full → build2 unchanged skips → build3 after a source edit recompiles). **Tier 2 (implemented): per-page partial rebuild.** The compile builds app-wide aggregates (imports/app-root/memo dedup) from all pages at once, so the integration (a) added per-page memo attribution to `CompileContext.compile()` (record which `auto_memo_components` each page first registers), and (b) in `compile_app`, validates each page against three inputs — its **own module source** (only the edited page misses), the **coarse `shared_fingerprint`** of everything shared except page and pure-state files (template/component/config edits → all miss), and the **content hashes of the fine-grained state files it actually uses** (a pure-state-module edit invalidates only the pages that reference that state — verified: editing one state file recompiles only its user, others reused). State usage is recovered from the page's output *and the memo components it owns* (auto-memoized stateful subtrees), since each stateful memo's owner regenerates it on recompile.

**Dev-reloader process model (checked) — and its consequence for Tier 2.** `reflex run` serves the backend via uvicorn/granian with `reload=True` (`exec.py:602,657`): the reloader is a *supervisor that runs the server in a worker subprocess and respawns it on every `.py` change*; the fresh worker re-imports the app → `app._compile()` → `compile_app()` (`app.py:1317`). So **each hot reload is a new process**, and Tier 2's in-memory `_PAGE_STORE` starts empty every time — **Tier 2 does not fire in the `reflex run` edit-loop** (it only helps within one long-lived process: programmatic `compile_app`, tests, a persistent compiler). Tier 1 (disk) survives, but only skips the *no-change* case; a `.py` edit forces a full compile in the fresh worker. **To win the edit-loop, Tier 3 must persist the per-page cache to disk**, which requires making each page's cached contribution *serializable* (output strings + import/memo data instead of live `root_component`/memo objects) and reconciling `app._pages` (skipped pages don't populate it today). Verify mode is the safety net for that refactor. Only miss pages run the pipeline; cached pages' contributions (output, `frontend_imports`, `app_wrap_components`, stateful flag, `memo_contributions`) are re-merged into the aggregates. In-process only — contributions hold live objects (`root_component`, memo defs) — so it accelerates same-process recompiles (the dev loop) while Tier 1 handles cross-process no-op rebuilds. **Verified:** all-hits reuse is **byte-identical** to a full compile, and editing one page module recompiles only it (others, incl. 404, reused). Layering note: `compile()` is in `reflex-base` (can't see the hybrid key), so Tier 2 is orchestrated from `reflex/compiler/compiler.py`. **Verify mode (implemented): `REFLEX_COMPILE_CACHE_VERIFY`** — when the cache reuses pages, also run a full no-cache compile and compare aggregates (per-page output, imports, memo, stateful, app-wraps); on any divergence it **warns, clears the page store, and falls back to the full result**, so a merge bug can never ship. Doubles compile time; opt-in to validate the cache on a given app (the right way to gain confidence on a memo-heavy app, which the test apps don't exercise). Verified end-to-end: a partial rebuild reports "incremental matches full compile"; the diff logic is unit-tested (identical / changed-page / missing-route+memo).

**Tier 3 (implemented, flag-gated): disk-persisted per-page cache — wins the edit loop.** `reflex/compiler/disk_cache.py` + `REFLEX_DISK_COMPILE_CACHE` (off by default). This is the Tier 3 the dev-reloader note above called for. After every full compile it writes `.web/reflex_compile_cache.json`: per page, its source fingerprint, the state files it uses (+ hashes), its rendered `output_code`/path, its `frontend_imports` (`ImportVar` is a frozen dataclass of primitives → JSON), the rendered memo files it owns, its app-wrap key-set, and `is_stateful`; app-wide, the shared fingerprint, fine-grained state hashes, reflex version, and merged imports. On a *fresh process* (the respawned hot-reload worker), `try_incremental_rebuild` reuses the manifest only when reflex version, route set, shared fingerprint, **and every state hash** match — restricting a fast rebuild to **pure page-body edits**, so all on-disk app-wide files (app root, contexts, theme, stylesheet) stay valid. Then it recompiles **only the source-changed pages** and rewrites just their files + owned memo files; everything else is reused untouched. The `app._pages` reconciliation the note flagged is resolved by the key insight that **`is_stateful` is true exactly when a page's first eval grew the state registry** (`app.py:992`): a *stateless* hit page is skipped entirely (runtime routing/load-events come from `add_page`/`_unevaluated_pages`, not `_pages` — verified at `app.py:1009,968`), while a *stateful* hit page is re-evaluated for the **backend only** (to re-register its state) but reuses its frontend file — so no state ever goes missing. **Guards → full-compile fallback (`return False`):** any global-input change, route add/remove, or a miss page that altered its app-wrap key-set or stateful flag (which would invalidate the reused app root). **Verified:** unit tests for the manifest round-trip / global-match / partition logic, plus two real-`App` integration tests (all-hits reuse; one-miss recompiles and writes byte-identical output for the changed page); the determinism property that makes reuse correct — an unchanged page recompiles byte-for-byte across builds — is asserted directly. Honest scope: v1 accelerates **page-body edits only**; a state edit, shared/template/config edit, route change, or version bump falls back to a full compile (correct, just not accelerated). `REFLEX_COMPILE_CACHE_VERIFY` remains the per-app backstop.

**Measured on the real docs app (417 pages, in-process at the `CompileContext` level, no npm).** Baseline cold compile: **54.6s** (101 pages reference state; only 3 *register new* state on eval). Disk cache, simulating the dev loop (edit one page → recompile only it, reuse the rest): **6.5s → 8.3×**, and it is correct by construction (it replays the prior compile's exact output + memo files, which are mutually consistent; reuse is per-process so each build is internally clean). The disk cache is the shippable win for the edit loop. **Component cache: a correctness bug found and fixed.** It cut the cold compile to 35.4s (1.54×) but the first measurement was **not** byte-identical to baseline — a real bug, not noise: a shared static subtree (the redirect `<head>` fragment: `<title>`+`<meta>`) is reused across the ~20 redirect pages, and the metadata-injection pass (`compiler.utils.add_meta`) mutated it **in place** (`page.children.extend(...)`) on each page, so by render time the fragment was duplicated ~20×. The `own()` copy-on-write the cache relied on covers the *plugin pipeline* but not that one pre-pipeline pass. **Fix:** made `add_meta` own-before-mutate (return a fresh copy with the meta appended instead of extending a possibly-shared children list); `_add_style_recursive` is idempotent for the app-global style, so it needs no change. After the fix, **cache-only divergence went from 62 pages → 0** (byte-identical to baseline). Regression test added (two routes with identical static bodies → shared tree + wrapper; asserts no `<title>` accumulation and byte-identity), confirmed to fail before the fix. A separate observation from the spike: running *many* compiles in one process is non-deterministic (≈288/417 pages differ at identical length — auto-memo content hashes shift as process-global generators advance across compiles), so byte-compare verification is only meaningful per fresh process, not across in-process recompiles; this does not affect the disk cache (cross-process, one clean compile each) and is orthogonal to the cache (it shows up baseline-vs-baseline too).

**In-process determinism — root causes and partial fix.** Investigating the ≈288-page in-process drift: it is *not* a single cause but several process-global generators whose state carries across compiles, each cascading because the auto-memo tag hashes the rendered subtree (so any drifting name inside a memo body re-hashes the memo, which re-renders every page that imports it). Sources found, in order of impact: (1) **the unique ref-name RNG** — `get_unique_variable_name` drew from a module-global seeded `Random` plus a `USED_VARIABLES` set, both persisting process-wide, so a second compile drew *different* `ref_*` names. **Fixed** (`reset_unique_variable_names`, called at the top of `CompileContext.compile`): the RNG reseeds and the used-set clears per compile, so names only need to be unique within a compile and reproduce across compiles. Drift 288 → 242 (46 pages), regression-tested. (2) **dynamic/component state class naming** — `ComponentState.create` names instances `<Name>_n<count>` from a monotonic per-class counter, and `_handle_local_def` disambiguates closure-defined states against `dir(reflex.istate.dynamic)`; both climb across compiles. This is the bulk of the remaining 242. A naive per-compile reset is unsafe — re-creating an already-registered substate name collides (`StateValueError`), and reusing the prior class leaks stale computed-var dependencies — so making it deterministic needs dynamic-substate *registration* to be idempotent across compiles (or position-based naming assigned during the tree walk), a focused state-machinery change deferred here. **Benign for production:** a single compile per process is internally consistent, and every real path compiles once per fresh process (production build, the dev-reload worker, each disk-cache rebuild), so output is deterministic where it matters; only multi-compile-in-one-process (verify mode, repeated test compiles) sees the residual drift.
...
**Component cache (implemented, flag-gated): the cold-build lever above.** `reflex/compiler/component_cache.py` + `REFLEX_COMPONENT_CACHE` (off by default). It memoizes *construction* of fully-static subtrees within one compile, so shared chrome (nav, footer, layout shells) is built once and reused across every page — saving the dominant `_post_init` cost (per-prop `LiteralVar.create` + `satisfies_type_hint`) **on a cold build**, which page caching cannot.

- **Interception point.** Component factories are pre-bound bound methods captured at import (`div = Div.create`), so patching `Component.create` cannot reach `rx.el.div(...)`. But `create` delegates to `cls._create(...)` via a *fresh attribute lookup*, so patching `Component._create` is reached by every `create` call. The cache key is computed there, **before** `_post_init` runs, so a hit skips construction entirely (the §80 "key before render" requirement).
- **Key.** A signature from the class + prop values + child signatures, propagated bottom-up. A subtree is cacheable only when every prop is a literal or a *constant* Var (no var data) and every child is cacheable; a state-bound Var, event handler, or component-valued prop yields no signature → that subtree (and its ancestors) are built normally. Static text leaves are special-cased: `Bare` wraps text via `_unsafe_create` (never hits `_create`), so its constant-`LiteralVar` `contents` is signed directly, keeping the chain alive through text.
- **Safety.** A reused subtree is shared across pages, but the pipeline clones-before-mutate via `PageContext.own()` (the existing cross-route sharing mechanism), so per-page mutation lands on clones. `REFLEX_COMPILE_CACHE_VERIFY` now also covers this cache: it runs a full *uncached* compile (the cache is uninstalled first) and falls back on any byte divergence.
- **Measured (synthetic, 100 pages with shared 20-link nav + 15-col footer, through the real `CompileContext`):** page compile **2.48× faster** (463 ms → 187 ms), 3861 hits / 539 builds, output **byte-identical** to the uncached compile. Unit + e2e tests in `tests/units/compiler/test_component_cache.py`.
- **Scope.** In-process only (caches live `Component` objects, reset per compile); like Tier 2 it accelerates same-process compiles, not the subprocess-respawning `reflex run` edit loop. The docs app itself couldn't be measured in-sandbox (it requires `reflex-enterprise` login).
