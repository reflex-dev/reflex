...
async def test_fully_controlled_input(fully_controlled_input: AppHarness):
    ...
    assert (await fully_controlled_input.get_state(token)).substates["state"].text == "ifoonitial"
    ...
    assert (await fully_controlled_input.get_state(token)).substates["state"].text == "overwrite the state"
